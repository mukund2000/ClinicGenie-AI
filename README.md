...
├── core/
│   ├── __init__.py
│   └── database.py              # Postgres/SQLite service layer
├── agent/
│   ├── __init__.py
...
- Seeds appointment slots from `data/doctor_availability.csv` when the appointment table is empty.
...
`core/database.py` is the main service and persistence layer. It owns all direct database access.

Responsibilities:

- Creates database tables.
- Initializes the database only once per process.
- Uses `DATABASE_URL` for Neon/Postgres when configured.
- Falls back to `data/clinicgenie.db` with SQLite for local development.
- Opens SQLite connections with foreign key support enabled when running in fallback mode.
...
- Seeds appointment slot data from CSV into the active database.
...
This means chatbot bookings and API bookings update the same active database.
...
By default, the app reads `DATABASE_URL` from `.env` when it starts. If the value starts with `postgres://` or `postgresql://`, `core/database.py` connects to Neon/Postgres using `psycopg`.
...
If `DATABASE_URL` is missing or not a Postgres URL, the app falls back to local SQLite at:

```text
data/clinicgenie.db
```

The same service functions are used in both modes.
...
```text
id          INTEGER PRIMARY KEY AUTOINCREMENT / GENERATED BY DEFAULT AS IDENTITY
name        TEXT NOT NULL
email       TEXT UNIQUE
phone       TEXT UNIQUE
dob         TEXT
...
This is important because patient lookup, onboarding, booking, cancellation, rescheduling, and history all update or read the same database.
...
SQLite foreign keys are enabled for every SQLite connection:

```python
connection.execute("PRAGMA foreign_keys = ON")
```

Postgres enforces the same relationships through table-level foreign keys.
...
This gives the active database a starting set of doctor availability records while allowing all future changes to happen through the service layer.

The service layer also has a lightweight guard, `ensure_database_initialized()`, used by database functions. It does not rebuild the schema on every call. It only calls `initialize_database()` if the process has not initialized the database yet. In SQLite fallback mode, it also reinitializes if `data/clinicgenie.db` no longer exists.
...
Streamlit API request timeouts are configurable:

```bash
$env:CLINICGENIE_API_TIMEOUT_SECONDS="20"
$env:CLINICGENIE_CHAT_TIMEOUT_SECONDS="120"
streamlit run client/streamlit_chatbot.py
```

Chat requests use the longer timeout because the first call may initialize the LangGraph agent and call the Groq model.
